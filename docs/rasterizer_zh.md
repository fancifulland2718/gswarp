...
`gswarp` 光栅化后端以纯 Python + NVIDIA Warp 实现了与 [diff-gaussian-rasterization](https://github.com/graphdeco-inria/diff-gaussian-rasterization) 等价的可微高斯光栅化管线，包含以下阶段：

1. **预处理**：3D 高斯投影到 2D，计算协方差、color（SH 评估）、AABB 瓦片矩形
2. **分箱**：高斯-瓦片映射、深度排序、瓦片范围标识
3. **前向渲染**：block_dim=256，协作式 tile 加载，alpha 从前到后混合
4. **反向渲染**：block_dim=32（单 warp），warp shuffle 梯度归约，atomic 写入减少 32 倍
5. **反向预处理**：means3D / scales / rotations / SH 反向传播

管线整体运行在同一个 PyTorch CUDA stream 上（通过 `_stream.py` 的 `ensure_aligned()` 对齐），与 PyTorch autograd 正确协作。
...
Warp 的 Python 层（类型检查、内核参数打包、从 torch tensor 转换为 `wp.array`）在高频调用（每个训练迭代）时会产生可观的 Python 函数调用开销。以下优化已应用于 `_rasterizer.py`：

**主要措施：**
- `GaussianRasterizationSettings` 使用 `dataclasses.dataclass` 而非普通类，字段访问从字典查找退化为属性查找
- `device` 对象在第一次调用时缓存，避免重复 `tensor.device` 查询
- 将参数验证和格式转换从调用热路径提前到 `_prep()` 中，减少条件分支
...
*以上为公共 API 端到端（forward + backward）稳态耗时，bit-exact 精度不变。*

30K 训练中的实际效果因场景而异：小场景（lego，~30 万高斯）可提升约 2.5%，大场景（truck、drjohnson，~200 万以上高斯）改善 < 0.1%——大场景的瓶颈在 GPU 核心执行，Python 层开销占比极低。
